...
//Insert into table 'relation' user parameters $winner and $loser
function mysql_insert_relation($userid,$winner, $loser){
	$con = mysql_connect("localhost","friendsh_nface","Xf%p4A3H74T8");
	$sql = "INSERT INTO relation (userid,winner, loser) VALUES ('$userid','$winner','$loser')";
	mysql_query($sql,$con) or die(mysql_error());
//	$sql = "INSERT INTO relation (winner, loser)
//	VALUES ('$winner','$loser')";
//	mysql_query($sql,$con);
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function novoteexceed($userid){
	$con = mysql_connect("localhost","friendsh_nface","Xf%p4A3H74T8");
	$result = mysql_query("SELECT userid FROM relation WHERE userid='$userid'",$con);
...
function winexists($pic1,$pic2){
	$con = mysql_connect("localhost","friendsh_nface","Xf%p4A3H74T8");
	$result = mysql_query("SELECT winner, loser FROM relation WHERE winner='$pic1' && loser='$pic2'",$con);
...
function loseexists($pic1,$pic2){
	$con = mysql_connect("localhost","friendsh_nface","Xf%p4A3H74T8");
	$result2 = mysql_query("SELECT winner, loser FROM relation WHERE winner='$pic2' && loser='$pic1'",$con);
...
function numberofwingamesby($pic){
	$con = mysql_connect("localhost","friendsh_nface","Xf%p4A3H74T8");
	$result = mysql_query("SELECT winner FROM relation WHERE winner='$pic'",$con);
	return mysql_numrows($result);
}

function numberoflosegamesby($pic){
	$con = mysql_connect("localhost","friendsh_nface","Xf%p4A3H74T8");
	$result = mysql_query("SELECT loser FROM relation WHERE loser='$pic'",$con);
	return mysql_numrows($result);
}

function totalgames(){
	$con = mysql_connect("localhost","friendsh_nface","Xf%p4A3H74T8");
	$result = mysql_query("SELECT winner FROM relation WHERE winner!='0'",$con);
	return mysql_numrows($result);
}
